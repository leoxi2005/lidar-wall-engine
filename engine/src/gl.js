...
//
// Deliberately dependency-free. Everything the engine draws is fullscreen triangles or
// point sprites, and pulling in a scene graph would buy nothing while adding the
// electron-builder trap where a package's `examples` folder is stripped out of app.asar
// and the packaged app boots to a black screen.
...
// wrapS is REPEAT by default: in a closed room the panorama's right edge physically
// touches its left edge, so a ripple leaving the last wall and arriving on the first is
// not a trick — it is the room's real topology. Rooms declared `wrap: false` should pass
// CLAMP_TO_EDGE instead.
